...
function getDegree(degreeindex, studyindex) {
    if (!studying) {
        ...
        if (studying) {
            console.log("You are already studying a degree.");
...
function doDegree() {
    if (studying) {
        ...
        if (degree_years < degree_length) {
            ...
            const years_left = degree_length - degree_years;
            ...
            salary += (education_levels[degree_index].areas_of_study[area_of_study_index].salary) / 12 / 30 / working_hours;
    ...
    if (type === "College") {
        for (let i = 0; i < education_levels[1].areas_of_study.length; i++) {
            ...
            degree.onclick = function () {
                // Push the button ID to the disabled array
                disabled_degree_buttons_id.push(degree.id);
                degree.disabled = true;
